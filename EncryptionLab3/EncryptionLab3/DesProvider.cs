...
namespace EncryptionLab3;

public static class DesProvider
{
    private static readonly byte[] _ip = 
        {
    ...
    private static readonly byte[] _fp = 
        {
    ...
    private static readonly byte[] _k1p = 
        {
            57, 49, 41, 33, 25, 17, 9, 1,
            58, 50, 42, 34, 26, 18, 10, 2,
            59, 51, 43, 35, 27, 19, 11, 3,
    ...
    private static readonly byte[] _k2p = 
        {
            63, 55, 47, 39, 31, 23, 15, 7,
            62, 54, 46, 38, 30, 22, 14, 6,
            61, 53, 45, 37, 29, 21, 13, 5,
    ...
    private static readonly byte[] _cp = 
        {
    ...
    private static readonly byte[] _ep = 
        {
            32, 1, 2, 3, 4, 5, 4, 5,
            6, 7, 8, 9, 8, 9, 10, 11,
    ...
    private static readonly byte[] _p = 
        {
    ...
    private static BitArray[] CreateSubKeys(byte[] keyBytes)
    {
        ...
        for (var i = 0; i < 28; i++)
        {
            left[27 - i] = keyBits[64 - _k1p[i]];
            right[27 - i] = keyBits[64 - _k2p[i]];
        }
    ...
    private static void FeistelCypher(Span<byte> messageBytes, IReadOnlyList<BitArray> subKeys, bool encrypt)
    {
        ...
        for (var i = 0; i < 32; i++)
        {
            left[31 - i] = messageBits[64 - _ip[i]];
            right[31 - i] = messageBits[64 - _ip[i + 32]];
        }
        ...
        for (var i = 0; i < 64; i++)
            messageBits[63 - i] = leftRight[64 - _fp[i]];
    ...
    private static void F(BitArray right, BitArray subKey)
    {
        ...
        for (var j = 0; j < 8; j++)
        {
            ...
            var value = _sBox[7 - j, row, column];
